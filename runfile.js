...
const c = {
  ...
  install(filename) {
    ...
    return `cd test_build && yarn cache list --pattern "gatsby-plugin-purgecss" && yarn cache clean "gatsby-plugin-purgecss" && yarn cache list --pattern "gatsby-plugin-purgecss" && yarn add -D file:../${filename} && yarn upgrade file:../${filename} && cd ..`;
  }
};
